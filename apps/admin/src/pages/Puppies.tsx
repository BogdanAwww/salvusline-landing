...
export default function Puppies() {
  ...
  async function handleReorder(reordered: PuppyRow[]) {
    setPuppies(reordered);
    const updates = reordered.map((p, i) => supabase.from("puppies").update({ sort_order: i }).eq("id", p.id));
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", gap: "0.75rem" }}>
        <div>
          <h1 style={{ fontSize: isMobile ? "1.25rem" : "1.5rem", fontWeight: 700 }}>Puppies</h1>
          <p style={{ color: "#666", fontSize: "0.9rem", marginTop: 4 }}>{puppies.length} puppies</p>
        </div>
        <Link to="/puppies/new" style={{ padding: "0.65rem 1.25rem", background: "#EC6B15", color: "#fff", borderRadius: 8, textDecoration: "none", fontWeight: 600, fontSize: "0.9rem", whiteSpace: "nowrap", flexShrink: 0 }}>
          + Add Puppy
        </Link>
      </div>
            ...
            isMobile ? (
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: "0.65rem" }}>
                  {puppy.photo_url ? (
                    <img src={puppy.photo_url} alt="" style={{ width: 44, height: 44, borderRadius: 6, objectFit: "cover", flexShrink: 0 }} />
                  ) : (
                    <div style={{ width: 44, height: 44, borderRadius: 6, background: "#111", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ color: "#666", fontSize: "0.7rem", marginTop: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {[
                        puppy.birth_date ? formatDate(puppy.birth_date) : null,
                        puppy.sire ? `S: ${puppy.sire}` : null,
                      ].filter(Boolean).join(" · ")}
                    </div>
                  </div>
                  <span style={{ fontSize: "0.7rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", color: STATUS_COLOR[puppy.status] ?? "#666", flexShrink: 0 }}>
                    {puppy.status}
                  </span>
                </div>
                <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.6rem" }}>
                  <button onClick={() => navigate(`/puppies/${puppy.id}`)} style={{ flex: 1, padding: "0.4rem", background: "#2a2a2a", color: "#ccc", border: "none", borderRadius: 6, fontSize: "0.8rem", cursor: "pointer" }}>Edit</button>
                  <button onClick={() => handleDelete(puppy.id)} style={{ padding: "0.4rem 0.65rem", background: "rgba(239,68,68,0.15)", color: "#f87171", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 6, fontSize: "0.8rem", cursor: "pointer" }}>Delete</button>
                </div>
              </div>
            ) : (
