  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
        <div>
          <h1 style={{ fontSize: "1.5rem", fontWeight: 700 }}>Dogs</h1>
          <p style={{ color: "#666", fontSize: "0.9rem", marginTop: 4 }}>{dogs.length} dogs registered</p>
        </div>
        <Link to="/dogs/new" style={{ padding: "0.65rem 1.25rem", background: "#EC6B15", color: "#fff", borderRadius: 8, textDecoration: "none", fontWeight: 600, fontSize: "0.9rem" }}>
          + Add Dog
        </Link>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        {dogs.map((dog) => (
          <div key={dog.id} style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 10, padding: "1rem 1.25rem", display: "flex", alignItems: "center", gap: "1rem" }}>
            {dog.cover_image_url && (
